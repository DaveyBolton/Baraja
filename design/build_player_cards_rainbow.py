...
# Rainbow cosmetic deck - same design/prompt/seed (1979140122) as the
# committed gold frame, "gold" swapped for a full-spectrum iridescent
# rainbow material (ArtEngine/jobs/baraja_card_frame_v15_rainbow.txt), same
# zero-margin fix technique. A purchasable whole-deck skin (Dave: "instead
# of skins...FRAMES!... in order to get new frames, you are actually just
# getting a whole deck") - not wired into the store/selection logic yet,
# this script only builds the asset set.
FRAME_PATH = r"C:\Dev\Stephen-AI-Studio\ArtEngine\out\baraja_card_frame_v15_rainbow\card_frame_v15_rainbow_zero_margin.png"
...
OUT_DIR = r"C:\Dev\Baraja\design\cards_rainbow"  # Spanish (default language)
OUT_DIR_EN = r"C:\Dev\Baraja\design\cards_rainbow_en"
...
# interior opening spans x 76-659, y 77-957 - close to but not identical to
# the gold frame's own interior (69-667, 58-979), since this generation's
# zero-margin fix used a different uniform scale factor (the raw generation
# had a slightly different residual margin than gold's). Re-measured rather
# than assumed. TITLE_CENTER_Y/BODY_GAP_BELOW_TITLE etc. are unchanged from
# gold - those are card-level design choices (10% above the card's own
# midline), not tied to this specific frame's pixel geometry.
ART_TOP = 80
ART_BOTTOM_MAX = 576
ART_MAX_W = 500
ART_OVERSIZE = 1.0
...
TITLE_LINE_GAP = 4
TITLE_STROKE = 2
TITLE_FILL = (230, 230, 235, 255)  # near-white so it reads against every hue of the rainbow border
TITLE_STROKE_FILL = (20, 10, 25, 255)
...
# Cost gem socket, top-left corner (measured visually on FRAME_PATH - the
# rainbow border's own hues made the gold/silver frames' color-based gem
# detection unreliable, since blue/purple show up everywhere on this
# border, not just at the gem).
COST_CX, COST_CY, COST_R = 92, 56, 56
COST_FONT_SIZE = 80

# Suit-medallion socket, bottom-center (measured on FRAME_PATH: outer
# decorative rim bulges to about radius 62, center found via the ring's
# widest horizontal cross-section near the bar).
MEDALLION_CX, MEDALLION_CY, SOCKET_R = 368, 966, 48
